...
def copy_file_to_neo4j_import(local_file_path: str, filename: str = None):
    """Copy a file to the Neo4j import directory (both local and container)

    Args:
        local_file_path: Path to the local file to copy
        filename: Optional filename to use in the import directory. If None, uses the original filename.

    Returns:
        The filename that can be used in Neo4j LOAD CSV commands
    """
    import shutil
    ...
    # Copy to local import directory
    local_import_dir = get_neo4j_import_dir()
    if local_import_dir:
        os.makedirs(local_import_dir, exist_ok=True)
        local_dest = os.path.join(local_import_dir, filename)
        shutil.copy2(local_file_path, local_dest)
        print(f"Copied {local_file_path} to {local_dest}")

    # Copy to Docker container
    container_dest = f"neo4j-adk:/var/lib/neo4j/import/{filename}"
    os.system(f"docker cp '{local_file_path}' '{container_dest}'")
    print(f"Copied {local_file_path} to Neo4j container: {filename}")
